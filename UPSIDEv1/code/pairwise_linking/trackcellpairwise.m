...
function tracked_matrix = trackcellpairwise(submatrix,t,D_now,Dlower_next,...
Dupper_next)
...
Dist_matrix_next = pdist2([submatrix_now.Xcenter,submatrix_now.Ycenter],...
[submatrix_next.Xcenter,submatrix_next.Ycenter]);
...
if ~isempty(pairs)
    ...
    pairs_good = pairs(~ismember(pairs(:,2),[unique(duplicate_value)',0]),:); 
    %remember having to remove 0 partner as well
else
    pairs_good = [0,0];
end
