...
class AML211Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        image_name = self.data_frame.iloc[idx,1]
        image = io.imread(image_name)
        image = np.reshape(image,(64,64,1)) # reshape to standard format (H,W,C)
        position = self.data_frame.iloc[idx,2]
        timepoint = self.data_frame.iloc[idx,3]
        obj = self.data_frame.iloc[idx,4]
        trial = self.data_frame.iloc[idx,7]
