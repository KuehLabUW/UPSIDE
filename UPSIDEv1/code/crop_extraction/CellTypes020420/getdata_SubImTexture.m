function getdata_SubImTexture(objects,pos,dirname,BF_name,wSynth_name,wCellTrace_name,seg_name)
...
for t = 1:numel(objects)
    ...
    subim_name = 'CellTypesShapes020420_pos%d_t%d_c%d.TIF';
    subim_Texture_name = 'CellTypesShapes020420_pos%d_t%d_c%d_texture.TIF';
    subim_Mask_name = 'CellTypesShapes020420_pos%d_t%d_c%d_mask.jpg';
    
    cd(dirname.script)
    %keyboard
    M = getsubim_Texture(objects,im_1,im_Synth,im_CellTrace,pos,t,subim_name,subim_Texture_name,subim_Mask_name,subdirname,seg_name);
 ...
 %% save MM as csv
 cd(dirname.script)
 csvname = sprintf('SubImTextureCellTypesShapes020420pos%d.csv',pos);
 %keyboard
 if numel(fieldnames(M)) > 0 %only add instances with nonzero number of objects
