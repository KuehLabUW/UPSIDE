...
function M = getsubim_Texture(objects,im_1,im_Synth,im_CellTrace,pos,t,subim_name,subim_Texture_name,subim_Mask_name,subdirname,seg_name)
    %keyboard
    %substract image to its background
    %rewrote this use cell trace image instead of DAPI
    eval(sprintf('segfunc = @%s;',seg_name));
    im_SynthBW = segfunc(im_CellTrace);
    
    im_1 = imresize(im_1,[1080,1080]);
    background = double(median(im_1(im_SynthBW)));
    
    im_1 = im_1 ./ background;
    im_CellTrace_dub =  im_CellTrace;
    %first watershed the celltrace to divide touching cells
    thresh = 0;
    im_CellTrace_dub(im_CellTrace_dub < thresh) = 0;
    h1 = fspecial('gaussian',[5 5], 2); % 2D guassian filter was 30
    i1 = imfilter(im_CellTrace_dub,h1);   % gaussian filtered image
        ...
        try %only save cells that are not at edges
            ...
            if numel(R) > 1
                ...
                try
                    ...
                    sub_im = [sub_im,zeros(small_dim,big_dim-small_dim)]; %add one column BF
                    sub_im = [sub_im;zeros(big_dim-small_dim,big_dim)]; %add one row BF
                catch
            ...
            cd('/media/phnguyen/Data2/Imaging/CellMorph/code/CellTypes112319/')
