...
RFPoffset = 2000;
P = 280;
dirname = '/data/phnguyen/Imaging/RawData/072017 - processed/';

for p = 1:P    % loop over all the stage positions
   pos(p) = load([dirname 'pos ' num2str(p) '/segtrackints.mat']);  % load all the objects
end

T = length(pos(1).objects);   % the number of timepoints
clear('objects');
...
for t = 1:T    % loop through all timepoints
    t
    obj = [];
    for p = 1:P   % loop through all stage positions
...
gatenames = {'ALL'};
save([dirname 'segtracksint_ALL072017.mat'],'objects','gatenames');
