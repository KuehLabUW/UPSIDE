...
import numpy as np
import sys
from CellTypesDataset_Texture import CellTypesDataset_Texture
...
global args
parser = argparse.ArgumentParser(description="texture feature learning algorithm")
...
#total data set has 3834728 images
trainset = CellTypesDataset_Texture(data_path,transform = ToTensorLIVE())
...
for step in range(total_step):
    ...
    #extract data
    images, position, timepoint, cell, trial = data_iter.next()
    ...
    TINY = 1e-15
    total_loss = F.mse_loss(X_sample, images)*recon_contribution + KLD_contribution*-0.5 * torch.sum(1 + logvar - mu.pow(2) - var.std())
    ...
    if (step+1) % 1000 == 0:
        ...
        torch.save(Q.state_dict(),weights_path + 'Q_VAE_CellTypes__large_LIVE_100z_nodropout_Texture_{}.pt'.format(step+1)) 
        torch.save(P.state_dict(),weights_path + 'P_VAE_CellTypes__large_LIVE_100z_nodropout_Texture_{}.pt'.format(step+1))
