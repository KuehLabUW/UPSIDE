...
for i = 1:T
    ...
    try
        ...
        images(1).im = zeros(1080,1080);
        images(2).im = zeros(1080,1080);
        images(3).im = zeros(1080,1080);
        images(4).im = zeros(1080,1080);
        d_count = d_count + 1;
        err_name = 'error_load_pos%d_t%d.mat';
        err_name = sprintf(err_name,pos,i);
        save(err_name,'i','pos')
        disp('ERROR')
    end
    count = 1;
