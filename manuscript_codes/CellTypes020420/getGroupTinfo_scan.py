...
##%
# load data
csvs_dirname = '/media/phnguyen/Data2/Imaging/CellMorph/data/CellTypes020420/csvs/ClusterScan/'
...
def get_corrmatrix(df_updated,trial):
    ...
    prop_list = ['area','eccentricity','sharpvalue']
...
for c in np.arange(1,40):
    ...
    featurelist =list(df.iloc[:,14:]);
    namelist = featurelist + ['trial','t','group','area','eccentricity','sharpvalue']
    prop_list = ['area','eccentricity','sharpvalue']
    ...
    corr_matrix[1,:,:],mean_matrix[1,:,:],std_matrix[1,:,:] = get_corrmatrix(df_updated,2)
