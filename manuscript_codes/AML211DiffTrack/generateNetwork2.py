...
df = pd.read_csv('cluster2_tracked.csv')
df = df.reset_index()

#%%
subdf_crit = (df['pos']>5) & (df['pos']<11) & (df['t']>0*20) & (df['t']<90*20+1) #1-5 || 6 - 10 
subdf = df[subdf_crit]
...
np.savetxt('/media/phnguyen/Data2/Imaging/CellMorph/data/AML211DiffTrack/csvs/PropAhRiTrial1.csv',AMN2,delimiter=",")
