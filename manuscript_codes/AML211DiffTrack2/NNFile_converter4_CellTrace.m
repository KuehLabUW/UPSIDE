...
for p =1
    for t = 1:10
       ...
       filename = '/media/phnguyen/Data2/pytorch_fnet-master/pytorch_fnet/results/3d/AllCellTypesCellTracetrain/testDiffTrack2/0%d';
       ...
       predict_name = 'EXPTrial2_w3Camera CellTrace_s%d_t%d.TIF';
...
for p =1
    for t = 11:numel(timepointsA)
       i = i + 1; %file name count  
       filename = '/media/phnguyen/Data2/pytorch_fnet-master/pytorch_fnet/results/3d/AllCellTypesCellTracetrain/testDiffTrack2/%d';
       filename = sprintf(filename,dirname(i));
       display(dirname(i))
       cd(filename)
       signal = imread('signal.tiff');
