...
for t = 1:numel(objects)
    ...
    subim_name = 'AML211DiffTrack2_pos%d_t%d_c%d.TIF';
    subim_Texture_name = 'AML211DiffTrack2_pos%d_t%d_c%d_texture.TIF';
    subim_Mask_name = 'AML211DiffTrack2_pos%d_t%d_c%d_mask.jpg';
 ...
 %% save MM as csv
 cd(dirname.script)
 csvname = sprintf('SubImTextureAML211DiffTrack2pos%d.csv',pos);
 %keyboard
 if numel(fieldnames(M)) > 0 %only add instances with nonzero number of objects
