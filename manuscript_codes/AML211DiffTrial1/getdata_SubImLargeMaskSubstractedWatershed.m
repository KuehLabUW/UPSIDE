function getdata_SubImLargeMaskSubstractedWatershed(objects,pos,dirname,BF_name,wSynth_name,wCellTrace_name)
...
for t = 1:numel(objects)
    ...
    subim_Mask_name = 'CellTypesShapes_pos%d_t%d_c%d_mask.jpg';
    
    cd(dirname.script)
    
    M = getsubim_LargeSubstractedMaskWatershed(objects,im_1,im_Synth,im_CellTrace,pos,t,subim_name,subim_Mask_name,subdirname);
 ...
 %% save MM as csv
 cd(dirname.script)
 csvname = sprintf('SubImLargeSubstractedWatershedCellTypesShapespos%d.csv',pos);
 if numel(fieldnames(M)) > 0 %only add instances with nonzero number of objects
