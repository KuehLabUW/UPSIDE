...
%csvfilename = 'CombinedMaskDirFluo.csv';
csvfilename = 'DEAD.csv';
cd(code_dir)


matrix = readtable(strcat(root_dir,csvfilename),'Delimiter', ',', 'HeaderLines', 0, 'ReadVariableNames', true, 'Format', '%f %s %f %f %f %f %f %f %f %f %f %f %f');
...
for cell = 1:numel(TotalDirname)
    ...
    if rem(numel(INDEX),100) == 0
        save('/media/phnguyen/Data2/Imaging/CellMorph/data/AML211DiffALL/mat/DEAD.mat','DEAD')
        save('/media/phnguyen/Data2/Imaging/CellMorph/data/AML211DiffALL/mat/INDEX.mat','INDEX')
        disp('saved')
        disp('File size:')
        disp(numel(INDEX))
        disp('%%%%%%%%%%%%')
