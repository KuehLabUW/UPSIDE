...
%% find enriched feature for each cluster
max_num = 3;
for cluster = [1:8]
    cluster_z_values = Z_matrix(:,cluster);
    [c_z_sorted,idx] = sort(cluster_z_values);
    %display the enriched features
    disp('cluster: ');disp(cluster);
    disp(Ytitle(idx(end-max_num:end)))
