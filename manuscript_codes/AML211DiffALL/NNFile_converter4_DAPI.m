...
for p =1
    for t = 1:10
       ...
       filename = '/media/phnguyen/Data2/pytorch_fnet-master/pytorch_fnet/results/3d/AllCellTypesDAPItrain/testDifferentiation/0%d';
       ...
       predict_name = 'EXPTrial1_w4Camera DAPI_s%d_t%d.TIF';
...
for p =1
    for t = 11:numel(timepointsA)
       i = i + 1; %file name count  
       filename = '/media/phnguyen/Data2/pytorch_fnet-master/pytorch_fnet/results/3d/AllCellTypesDAPItrain/testDifferentiation/%d';
       filename = sprintf(filename,dirname(i));
       display(dirname(i))
       cd(filename)
       signal = imread('signal.tiff');
...
for p =1:numel(positionsB)
    for t = 1:numel(timepointsB)
       ...
       %save image with MM name
       cd '/media/phnguyen/Data2/TempDAPI/'
       predict_name = 'EXPTrial2_w4Camera DAPI_s%d_t%d.TIF';
...
for p =1:numel(positionsC)
    for t = 1:numel(timepointsC)
       ...
       %save image with MM name
       cd '/media/phnguyen/Data2/TempDAPI/'
       predict_name = 'EXPTrial3_w4Camera DAPI_s%d_t%d.TIF';
