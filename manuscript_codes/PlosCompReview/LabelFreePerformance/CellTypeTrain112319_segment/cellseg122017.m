...
log_threshold = -10;  % edge-detection threshold for finding cell boundaries was -1
low_in = 4100/18000; % lowest intensity input value for cell regonition 
high_in = 4600/18000; % higest intensity input value to guarantee cell
low_out = 0; % adjusted low intensity output
high_out = 1; % adjusted high intensity output
minsize = 500;   % minimum size of a cell
...
i2 = imadjust(i1./18000,[low_in; high_in],[low_out; high_out]); % adjust image based on intensity thresholds
i3 = imfilter(18000.*i2,h2);   % the laplacian of the gaussian filtered image
...
%subplot(3,3,1); imshow(i1,[]) % original
%subplot(3,3,2); imshow(i2,[]) % gaussian filtered
%subplot(3,3,3); imshow(i3,[]) % laplacian of gaussian
%subplot(3,3,4); imshow(i4,[]) % negative values of laplacian
%subplot(3,3,5); imshow(i5,[]) % eroded and dilated
%subplot(3,3,6); imshow(i6,[]) % clear border objects
