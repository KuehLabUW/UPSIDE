...
umapfilename_drug = 'AnnoSemiCombinedSubstractedDirFluoUMAPdrug_onehot_large2.csv';
umapfilename_nondrug = 'AnnoSemiCombinedSubstractedDirFluoUMAPnondrug_onehot_large2.csv';
cd(code_dir)


%read bulk csv file
matrix_nondrug = readtable(strcat(root_dir,umapfilename_nondrug),'Delimiter', ',', 'HeaderLines', 0, 'ReadVariableNames', true, 'Format', '%s %f %f %f %f %f %f %f %f %f %f %f');
matrix_drug = readtable(strcat(root_dir,umapfilename_drug),'Delimiter', ',', 'HeaderLines', 0, 'ReadVariableNames', true, 'Format', '%s %f %f %f %f %f %f %f %f %f %f %f');
...
array_nondrug_nonUM = bundle_array3D(matrix_nondrug_nonUM,3,11,12); %get time data , xumap, yumap and put into an array format

objects = bundle_mat_forcyto2(matrix_nondrug_nonUM,11,12); %bundle the table into a .mat format for visualization
%%
matrix_drug_nonUM = matrix_drug(matrix_drug.pos <= 16,:);

array_drug_nonUM = bundle_array3D(matrix_drug_nonUM,3,11,12); %get time data , xumap, yumap and put into an array format

objects = bundle_mat_forcyto2(matrix_drug_nonUM,11,12); %bundle the table into a .mat format for visualization
...
for t = 1:numel(Tseries)
   ...
   subarray = bundle_array3D(submatrix,3,11,12);
...
p = 1;
for t = 1:20:1761
    submatrix =  matrix_nondrug_nonUM(matrix_nondrug_nonUM.t==t,:);
    subarray = bundle_array3D(submatrix,3,11,12);
    Tmatrix(t).xumap = subarray(:,1);
    Tmatrix(t).yumap = subarray(:,2);
    Tmatrix(t).t     = subarray(:,3);
...
disp(p);
for t = 1:20:981
    submatrix =  matrix_drug_nonUM(matrix_drug_nonUM.t==t,:);
    subarray = bundle_array3D(submatrix,3,11,12);
