...
umapfilename = 'CombinedSubstractedDirUMAP_mixgauss_largeLIVE60z_nodropout.csv';
...
array_nondrug_nonUM = bundle_array3D(matrix_nondrug_nonUM,6,8,9); %get time data , xumap, yumap and put into an array format

objects = bundle_mat_forcyto2(matrix_nondrug_nonUM,8,9); %bundle the table into a .mat format for visualization
%%
matrix_drug_nonUM = matrix_drug(matrix_drug.pos <= 16,:);

array_drug_nonUM = bundle_array3D(matrix_drug_nonUM,6,8,9); %get time data , xumap, yumap and put into an array format

%objects = bundle_mat_forcyto2(matrix_drug_nonUM,8,9); %bundle the table into a .mat format for visualization
objects = bundle_mat_forcyto2(matrix_drug_nonUM,8,9); %bundle the table into a .mat format for visualization
